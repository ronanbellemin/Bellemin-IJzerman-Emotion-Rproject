...
df <- readxl::read_xlsx("data_explore_all_copie.xlsx")
...
my_sim_data <- function(
        ...
        #n_low_arousal  =  5,   # number of low arousal stimuli (quadruple)
        #n_high_arousal =  5,   # number of high arousal stimuli (quadruple)
        n_emo  =  3, # number of emo stimuli
        n_relation = 5, # number of relational stimuli
        beta_0     =  mean(vec),   # grand mean
        beta_1     =  .3,   # Main effect of emotions
        beta_2     =  .3,   # Main effect of RMs
        beta_3     = .1,    # interaction effect
    ...
    items <- data.frame(
        ...
        category1 = rep(c("happy","anger","sadness"), n_relation),
        category2 = rep(c("communal", "authority1","authority2","equality","market"), n_emo),
    ...
    crossing(subjects, items)  %>%
        mutate(e_si = rnorm(nrow(.), mean = 0, sd = sigma),
               Temp = beta_0 + T_0s + O_0i + (beta_1 + T_1s) * X_i + (beta_2 + T_1s) * Y_i +
                   (beta_3 + T_1s)*(X_i*Y_i) + e_si) %>%
        select(subj_id, item_id, category1, X_i, category2, Y_i, Temp)
}
...
single_run <- function(...) {
    ...
    dat_sim <- my_sim_data(...)
    mod_sim <- lmer(Temp ~ X_i*Y_i + (1 | subj_id), dat_sim)
...
pwr_df
save(pwr_df, file = "EF=0.1&N=25_simpleeffect.Rdata")
#dat_sim <- my_sim_data()
#mod_sim <- lmer(Temp ~ X_i + (1 | item_id) + (1 + X_i | subj_id), 
#dat_sim, control = lmerControl(optimizer ="Nelder_Mead"))
#pwr1 <- simr::powerSim(mod_sim, nsim = 100, progress = F)
